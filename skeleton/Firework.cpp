...
Firework::Firework(Vector3 Pos, Vector3 Vel, double Damp, Vector3 Grv, double time, std::vector<Payload> p, int type, std::vector<FireworkRule> f, Vector4 col) : ReproducingParticle(Pos, Vel, Damp, {0.0, 0.0, 0.0}, time, col)
{
	rules = f;
	payloads = p;
	fireworkType = type;
